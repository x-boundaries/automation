...
- `scripts/ac2_member_expiry_capability_probe.ps1` (with pure helper library `scripts/member_expiry_capability_probe_lib.ps1`): explicitly gated, inactive-by-default synthetic capability probe that creates exactly one synthetic member with `ExpiryDate=2028-06-30`, verifies the value after a `GetMember` read-back, and never updates, deletes, rolls back, or cleans up. Before the irreversible save it atomically creates a permanent single-use attempt claim in the required `-StateDirectory` (never overwritten or deleted, so crashes and concurrent launches fail closed), binds the durable non-overwriting evidence to the run via `operation_id`, `-ApprovalReference`, and a non-secret target fingerprint, reports honest terminal outcomes (a confirmed save with a failed read-back is `WRITE_CONFIRMED_READBACK_FAILED`, never a pre-write failure), and returns exit `0` only for `EXPIRY_VERIFIED`. It exists only to prove ExpiryDate persistence before the main creation UAT enables it; it requires owner approval naming the AutoCount target and one synthetic record, and any uncertain save is terminal and never retried. See [Synthetic ExpiryDate capability probe runbook](docs/autocount2-automation/member_expiry_capability_probe_runbook.md).
