...
Import round-trip fidelity does not extend to UI binding. The 2026-07-15 live UAT established (and the n8n editor implementation confirms) that when the sheet locator (`sheetName.value`) of a Google Sheets 4.7 node changes in the n8n editor, the editor resets that node's column resource mapper: the defined column values are cleared, the match column is re-defaulted, and hidden options (Cell Format, `returnFirstMatch`) can be lost. A template that ships an empty sheet locator therefore always loses its Update-node mappings the moment the operator picks the tab. To prevent this, all three Google Sheets nodes ship the sheet locator in `By Name` mode with the non-secret placeholder `REPLACE_WITH_SOURCE_TAB_NAME`. The operator replaces the placeholder with the source tab title in a local import copy before importing, and binds only the Google credential and the spreadsheet in the n8n UI. The column resource mapper depends only on the sheet locator, not the spreadsheet locator, so binding the spreadsheet does not reset the mapping. The Update node also ships a deliberately narrow cached mapper schema (header names and mapper flags only, no values or locators) that lists only the columns the mapper actually uses: the match column `Gate5AApprovedForMapping`, the eight controlled review/status columns, and the virtual `row_number` entry (readOnly, removed). It intentionally pins no unrelated intake/source headers. The cached schema is a UI rendering cache, not a runtime contract: the Google Sheets v4.7 `update` operation stopped enforcing the cached schema against the live header row in n8n PR #10201 (`checkForSchemaChanges` was removed from the update path; since PR #30621 the remaining guard in other operations is set-membership based, not positional). At runtime the update depends only on the match column and the mapped review columns existing verbatim in the bound tab, and the 2026-07-17 successful Gate 5A UAT run confirmed this on the live instance. The actual current operational UAT source tab keeps Google-Forms-era intake headers (for example `Timestamp` and a `Marketing Consent` header that currently carries two trailing ASCII spaces) that differ from the documented Gate 4A canonical source-header aliases; the identity Code node reads the intake fields it needs through its documented field aliases (for example `Mobile Number` for the member number) and never reads the timestamp header. Never rename or reformat the live sheet headers to match documentation, and never widen this committed schema to pin intake/source headers: pinned source headers previously caused a canonical-alias schema that mismatched the proven operational tab.
