...
Research checked: 2026-06-27 SGT.
...
This PR is discovery, planning, and dry-run validation only. It does not implement production writeback, does not create an n8n production workflow, and does not write to AutoCount SQL tables.
...
- **Desktop assembly API: confirmed for AutoCount Accounting 2.0 member maintenance.** This is the strongest evidence path for the installed local AutoCount system because the wiki shows v2 code using AutoCount assemblies directly.
...
- Installed AutoCount Accounting 2.x exact version/build on the X-Boundaries machine.
- Which local DLLs contain `AutoCount.GeneralMaint.MemberMaintenance` in the installed environment.
...
| https://wiki.autocountsoft.com/wiki/Programmer:Member_v2 | Programmer:Member v2 | AutoCount 2.0 member maintenance sample uses `MemberCommand`, `NewMember(false)`, `GetNextMemberNo`, `SaveMember`, `GetMember`, and `DeleteMember`. | Confirmed |
