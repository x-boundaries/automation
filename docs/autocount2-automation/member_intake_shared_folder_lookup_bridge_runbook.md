...
Probe and temp cleanup remain ownership-verified by device/inode identity: a pathname is unlinked only while it still refers to the object this run created. A cleanup ownership mismatch fails closed, leaves the foreign or replacement entry byte-for-byte and identity untouched (never deleted, repaired, renamed, or overwritten), and retains the execution claim. Like every other filesystem classification in the runner, this check is ordered best-effort and is not race-proof: POSIX provides no atomic conditional unlink, and none is claimed here.
...
| POSIX preflight `cleanup_unconfirmed` (capability proven; probe cleanup incomplete, probe identity mismatch after foreign replacement, or durability unconfirmed) | `needs_fix`, zero lookups, foreign/replacement entry untouched, nothing retried or repaired, execution claim retained so reruns stay blocked |
