...
param(
    ...
    [switch]$EnableMemberNoSaveSchemaProbe,
...
if (-not $EnableMemberNoSaveSchemaProbe) {
    $refusal = [ordered]@{
        mode = "member-no-save-schema-probe"
        probe_enabled = $false
        refused = $true
        message = "Explicit opt-in is required. Re-run with -EnableMemberNoSaveSchemaProbe to run the no-save schema probe."
    }
    $refusal | ConvertTo-Json -Depth 4
    throw "Refusing to run live no-save member schema probe without -EnableMemberNoSaveSchemaProbe."
}

if ($MaxColumns -lt 1) {
    throw "MaxColumns must be 1 or greater."
}
...
Write-Warning "AC2 member no-save schema probe is explicit opt-in. It creates an in-memory member entity only, emits sanitized schema metadata, and does not browse existing members or write data."
...
function Convert-ColumnSchema {
    param([System.Data.DataColumn]$Column)

    $defaultValue = $Column.DefaultValue
    $hasDefault = $false
    if ($null -ne $defaultValue -and $defaultValue -ne [System.DBNull]::Value) {
        $hasDefault = $true
    }

    return [ordered]@{
        column_name = $Column.ColumnName
        data_type = $Column.DataType.FullName
        allow_db_null = $Column.AllowDBNull
        max_length = $Column.MaxLength
        read_only = $Column.ReadOnly
        default_value_present = $hasDefault
    }
}
...
function Get-OrderedColumns {
    ...
    $ordered = New-Object System.Collections.Generic.List[System.Data.DataColumn]
    foreach ($columnName in $preferredColumnOrder) {
        if ($byName.ContainsKey($columnName)) {
            $ordered.Add($byName[$columnName])
        }
    }

    foreach ($column in $Columns) {
        if (-not $preferredColumnOrder.Contains($column.ColumnName)) {
            $ordered.Add($column)
        }
    }
...
try {
    ...
    if (-not $result.authentication_success -or -not $result.user_session_available) {
        throw "Authentication/session did not become available; member schema probe was not attempted."
    }
    ...
    if ($null -ne $memberEntity) {
        ...
        $schemaTable = $null
        if ($memberTable -is [System.Data.DataTable]) {
            $schemaTable = $memberTable
        }
        elseif ($memberRow -is [System.Data.DataRow]) {
            $schemaTable = $memberRow.Table
        }

        if ($null -ne $schemaTable) {
            $result.column_count = $schemaTable.Columns.Count
            $orderedColumns = @(Get-OrderedColumns $schemaTable.Columns $MaxColumns)
            $result.columns = @($orderedColumns | ForEach-Object { Convert-ColumnSchema $_ })
            $result.intake_relevant_columns_seen = @(
                $preferredColumnOrder | Where-Object { $schemaTable.Columns.Contains($_) }
            )
        }
    }
}
...
if (-not [string]::IsNullOrWhiteSpace($JsonOut)) {
    ...
    Write-Host "Wrote sanitized member no-save schema probe JSON to $jsonOutPath"
}
