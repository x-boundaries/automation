...
- Local deployment-bound deviation: import supports mutually exclusive `-WorkflowFile n8n-workflows\member_forms_gateway_ingest.workflow.json` mode. It selects exactly that immediate canonical child, blocks ambiguous/archived/active/scheduled live targets, captures an existing exact-target preimage, preserves its ID on update, and verifies the exact inactive/manual readback without restarting n8n. Broad mode retains its existing behavior. Live use still requires explicit current-turn approval.
