...
- Local deployment-bound deviation: import supports mutually exclusive `-WorkflowFile n8n-workflows\member_forms_gateway_ingest.workflow.json` mode with a required private `-BindingManifestFile`. It uses metadata-only discovery, exports only an exact selected target, blocks case/duplicate/name-ID ambiguity plus archived/active/scheduled targets, applies the manifest's exact endpoint/question/credential contract on both create and update, and verifies the complete locked workflow contract after import. Before mutation it persists immutable ignored recovery evidence under `.n8n-local/member-gateway-recovery/`; bounded mode never refreshes shared credential bindings. Broad mode retains its existing behavior. Live use still requires explicit current-turn approval.
