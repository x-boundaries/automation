...
class MemberIntakeN8nDryRunDocsTests(unittest.TestCase):
    ...
    def test_queue_contract_defines_gate4a_real_queue_write_preparation(self):
        ...
        for phrase in [
            ...
            "The current actual source headers are `Date & Time`, `Full Name`, `AutoCount MemberNo`, `Email Address`, `Birthday Month`, `Marketing Consent`, and `PDPA Acknowledged`",
            "Gate 4A does not require `Gate4A Source Reference` or `Gate4A Source Row Ref` sheet columns",
            "queue `source_reference` and `source_row_ref` are derived internally",
            "The only Gate 4A operator approval marker in the sheet is `Gate4AApprovedForLookup`",
            "Gate 4A maps `Full Name` to required source name",
            "`AutoCount MemberNo` to the submitted member number and AutoCount `MemberNo` lookup value",
            "`Email Address` to required source email",
            "`Birthday Month` to the required birthday-month source field",
            "`Marketing Consent` to optional source metadata only",
            "`PDPA Acknowledged` to the mandatory PDPA source field",
            ...
            "must not copy names, emails, raw phone/member values, birthday values, raw marketing consent values, or raw submitted member values into the lookup queue",
            ...
            "overwrite that staged file on each manual run so stale rows from previous tests cannot accumulate",
            "`queue_row_count = 1`",
